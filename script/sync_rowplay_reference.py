...
``script/export_rowplay_native_parity.mjs``.  This script validates both sets
of outputs when present (and ``--check`` requires them).
"""
...
def main() -> None:
    ...
    # Parity fixtures are generated by export_rowplay_native_parity.mjs; the
    # check gate verifies their pin.
    parity_dir = REFERENCE_ROOT / "parity"
    if check:
        expected_parity = [
            "replay-motion-parity.json",
            "replay-grip-parity.json",
            "replay-equipment-parity.json",
            "replay-environment-parity.json",
            "replay-camera-parity.json",
            "replay-2d-parity.json",
        ]
        for name in expected_parity:
            fixture_path = parity_dir / name
            if not fixture_path.exists():
                mismatches.append(f"missing: parity/{name} (run export_rowplay_native_parity.mjs)")
                continue
            fixture = json.loads(fixture_path.read_text())
            if fixture.get("sourceCommit") != resolved:
                mismatches.append(f"parity/{name} pinned to a different commit")

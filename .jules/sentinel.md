...
**Prevention:** Bound the length of input strings globally in generic redaction utilities. For logging, if an input exceeds a safe threshold (e.g., 16KB / 16384 characters), return a placeholder like `[REDACTED_OVERSIZE]` instead of evaluating regexes, as large payloads are not only dangerous to process but also typically contain data that should not be logged raw anyway.

...
**Vulnerability:** The `URLSessionHTTPTransport` used `URLSessionConfiguration.default`, which implicitly uses the default `URLCache` and `HTTPCookieStorage`. This configuration may write HTTP responses (including those containing sensitive workout data) and credentials (like Bearer tokens in headers) to the device's persistent disk cache. If a device is compromised, this data could be extracted.
**Learning:** Default URLSession configurations silently persist data to disk, which violates the security requirement that sensitive tokens and payloads (like those from Concept2 API) are held in memory only and not leaked to the file system.
**Prevention:** Use `URLSessionConfiguration.ephemeral` when interacting with APIs that require bearer tokens and return sensitive user data. This guarantees that caches, cookies, and credentials are only held in memory and never written to disk.

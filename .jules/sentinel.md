...
## 2026-07-24 - DoS Prevention via Bounded File Reads
**Vulnerability:** The HR sample importer used `Data(contentsOf: url)` to read user-provided files into memory. This unbounded read could be exploited by selecting an extremely large file, leading to memory exhaustion and a Denial of Service (DoS) crash.
**Learning:** `Data(contentsOf:)` reads the entire file into memory at once and should never be used for files of unknown or potentially unbounded size, especially those provided via user input (like file pickers).
**Prevention:** Use `FileHandle(forReadingFrom:)` to read files up to a strict maximum size limit (e.g., 5 MB). Check if the data exceeds the limit and throw an error to fail securely, preventing memory exhaustion.

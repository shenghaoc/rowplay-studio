...
- Phase 10B validation on the ready-for-review branch passes all focused rival factory, parser, race-result, report, card, ghost-workflow, and 3D tests; `swift build --target RowPlayCore`; full `swift test` (949 tests, two expected authenticated-smoke skips); full `swift build`; both architecture scans; and `git diff --check`.

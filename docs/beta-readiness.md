...
- **3D replay foundation**: `ReplayStrokePose` (renderer-neutral pose model), `ReplayCourseLayout` (400m loop), `RealityReplaySceneView` (RealityKit scene), articulated sport rigs with joint hierarchies and contact invariants (RowErg, SkiErg, BikeErg), 2D/3D mode selector in replay view. Interactive cameras, quality tiers, and surface effects remain in Phase 8C+.

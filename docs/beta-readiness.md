...
- **3D replay foundation**: `ReplayStrokePose` (renderer-neutral pose model), `ReplayCourseLayout` (400m loop), `RealityReplaySceneView` (RealityKit scene), sport-specific low-poly procedural placeholders (RowErg, SkiErg, BikeErg), 2D/3D mode selector in replay view. Final athlete rigs, interactive cameras, and quality tiers remain in Phase 8B+.

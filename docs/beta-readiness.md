...
- Phase 10B validation on the final hardening tree passes the focused rival factory (15), parser (48), race-result (6), report (7), card (9), ghost-workflow (29), 3D scene-effects (13), and quality-scene (18) tests; `swift build --target RowPlayCore`; full `swift test` with 1,161 tests total (973 Core, 67 Platform, 121 Studio), two expected authenticated-smoke skips, and zero failures; full `swift build`; both architecture scans; and `git diff --check`.

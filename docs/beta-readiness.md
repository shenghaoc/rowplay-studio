...
- Phase 10B validation on the ready-for-review branch passes all focused rival factory, parser, race-result, report, card, ghost-workflow, and 3D tests; `swift build --target RowPlayCore`; full `swift test` (952 tests, two expected authenticated-smoke skips); full `swift build`; both architecture scans; and `git diff --check`.
- Phase 10B staged-bundle gates pass on the current hardening head with `./script/build_and_run.sh --verify`, `--automation`, and `--sign-verify`. A prior ready head was exercised through invalid and valid constant-pace selection, an actual CSV import via the native file panel, 2D and 3D rival rendering, finish/DNF verdict presentation, and real privacy-minimized race-report JSON and 1080x1440 race-card PNG saves. The current head adds security-scope/cancellation integration coverage and a visually inspected metric-rich card render with a fixed-canvas clipping regression test. Its native file-panel walkthrough could not be repeated because the Mac session was locked; TCX/FIT behavior remains covered by parser tests rather than claimed as a separate native file-panel walkthrough.

...
| `src/routes/replay/[id]` (UI and quality selection) | `Sources/RowPlayStudio/Views/ReplayView.swift` + `Sources/RowPlayStudio/Views/Replay3D/RealityReplaySceneView.swift` | Phase 3 adds the SwiftUI Canvas replay surface with playback controls. Phase 8D adds the accessible 3D-only quality menu and adaptive-reduction status, plus raw-before-clamp frame sampling while preserving clamped replay motion and the unchanged 2D UI. |

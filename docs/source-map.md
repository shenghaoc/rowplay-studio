...
| `src/lib/server/db.ts` (workout cache) | `Sources/RowPlayCore/Sync/WorkoutCache.swift` | Phase 4 ports workout cache as `WorkoutCache` protocol + `InMemoryWorkoutCache`; SQLite deferred to follow-up. |

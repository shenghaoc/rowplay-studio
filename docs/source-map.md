...
| `src/lib/replay/sources.ts` | `Sources/RowPlayCore/Replay/ReplayRivalFactory.swift` + `ReplayRivalFileParser.swift` | Phase 10B ports constant-pace ghost creation and CSV/TCX/FIT rival import without browser File/DOM APIs. Native import adds bounded reads, streaming quoted CSV, strict namespace-insensitive XML parsing with encoding-independent DTD rejection, exact constant-pace/trace-aware identities, and declared-payload/definition/field-bounded FIT checks. |
| `src/lib/replay/replayGap.ts` (finish behavior) | `Sources/RowPlayCore/Replay/ReplayRaceResult.swift` | Phase 10B computes finished race outcomes. Native distance races use **interpolated target crossing** rather than array endpoint timestamps (stricter than web endpoint comparison). |
| `src/lib/replay/raceCard.ts` | `Sources/RowPlayCore/Replay/ReplayRaceReport.swift` + `Sources/RowPlayStudio/Views/ReplayRaceCardView.swift` | Phase 10B local privacy-safe race report JSON and native race-card PNG (ImageRenderer), including sanitized rival distance/time/pace metrics as additive optional version-1 fields; no imported filenames, internal IDs, or public URL. |
| `src/routes/replay/[id]` (candidate lookup and route wiring) | `Sources/RowPlayPlatform/WorkoutLibrary.swift` + `Sources/RowPlayStudio/Views/ContentView.swift` | Phase 10A derives ranked local candidates once in the platform library cache and passes the fixed snapshot through the existing replay navigation route. |
...
| `src/lib/replay/replayRenderer.ts` (`PerfGovernor`, `applyPerfLevel`) | `Sources/RowPlayCore/Replay/ReplayPerformanceGovernor.swift` + `Sources/RowPlayStudio/Views/Replay3D/ReplayPerformanceController.swift` | Phase 8D ports median cadence calibration, capped working budget, EMA sampling, sustained-over-budget degradation, and post-step grace. The selected tier is a ceiling; degradation is sticky and one-way for the scene, with no automatic upgrade. |
| (new — no web equivalent) | `Sources/RowPlayCore/Replay/ReplayPerformanceMetrics.swift` + `Sources/RowPlayStudio/Views/Replay3D/ReplayPerformanceTelemetry.swift` | Phase 8D adds a scalar 120-sample accumulator and bounded unified logging for selection, degradation, and completed metric windows. It retains no unbounded history and logs no workout, account, token, filename, or stroke data. |
...
| `src/lib/replay/renderer3d.ts` (CourseRenderer3D) | `Sources/RowPlayStudio/Views/Replay3D/RealityReplaySceneView.swift` | Phase 8A ports the 3D course renderer concept to RealityKit `RealityView` on macOS. Phase 8D keys the inner RealityView by effective quality, pairs one raw playback interval with one measured scene update per tick generation, and rebuilds only for manual selection or sticky degradation. |
| `src/lib/replay/renderer3d.ts` (avatar builders) | `Sources/RowPlayStudio/Views/Replay3D/ReplaySportRig.swift` + `ReplayAthleteRig.swift` + `ReplayRowerRig.swift` + `ReplaySkiErgRig.swift` + `ReplayBikeErgRig.swift` + `ReplayMeshFactory.swift` | Phase 8A ports sport-specific low-poly avatar builders; Phase 8B upgrades to articulated rigs with joint hierarchies, contact invariants, and tapered limbs. |
| `src/lib/replay/renderer3d.ts` (tapered limbs, hands, feet, head) | `Sources/RowPlayStudio/Views/Replay3D/ReplayMeshFactory.swift` | Phase 8B ports tapered limb, hand, foot, head, and wheel mesh helpers. |
| `src/lib/replay/renderer3d.ts` (contact anchors, IK) | `Sources/RowPlayCore/Replay/ReplayRigPose.swift` | Phase 8B adds renderer-neutral rig-pose model with sport-specific joint angles, contact positions, and deterministic solver. |
| `src/lib/replay/renderer3d.ts` (ghost materials) | `Sources/RowPlayStudio/Views/Replay3D/ReplaySportRig.swift` | Phase 8B adds recursive ghost translucency to every rig material. |
| `src/lib/replay/renderer3d.ts` (scene/course) | `Sources/RowPlayStudio/Views/Replay3D/Replay3DSceneBuilder.swift` | Phase 8A ports the static scene builder (course ring, start/finish, lighting, camera). Phase 8D applies each effective tier's exact course-ring and lane-marker counts while retaining eight distance markers and the start/finish marker. |
| `src/lib/replay/renderer3d.ts` (chase camera and speed-aware FOV) | `Sources/RowPlayCore/Replay/ReplayCamera.swift` + `Sources/RowPlayStudio/Views/Replay3D/ReplayCameraController.swift` | Phase 8C keeps finite frame-rate-independent chase-camera damping and the 46...51 degree FOV range, then adds side, overhead, and clamped orbit presets. Camera solving follows the native 400m `ReplayCourseLayout`, not the web renderer's 1km circle. |
| `src/lib/replay/renderer3d.ts` (`WakeTrail`) | `Sources/RowPlayCore/Replay/ReplayEffects.swift` + `Sources/RowPlayStudio/Views/Replay3D/ReplayEffectRenderer.swift` | Phase 8C ports bounded live and ghost foam/snow trail history on the native 400m lanes, with lower ghost opacity and explicit paused/seek/reduced-motion reset semantics. Phase 8D makes the fixed capacity quality-aware from zero at low through 44 entries per participant at ultra. |
| `src/lib/replay/motion.ts` (`ParticlePool`) | `Sources/RowPlayCore/Replay/ReplayEffects.swift` + `Sources/RowPlayStudio/Views/Replay3D/ReplayEffectRenderer.swift` | Phase 8C ports fixed-capacity spawn, integration, gravity, fade, swap-expiry, clear, and full-pool behavior into prebuilt RealityKit entities. Phase 8D makes allocation quality-aware from zero at low through 72 particles at ultra and updates only active prefixes/newly inactive tails. |
| `src/lib/replay/motion.ts` (`catchEvents`) + `src/lib/replay/renderer3d.ts` (sport profiles and catch spray) | `Sources/RowPlayCore/Replay/ReplayMotion.swift` + `Sources/RowPlayCore/Replay/ReplayEffects.swift` + `Sources/RowPlayStudio/Views/Replay3D/ReplayEffectRenderer.swift` | Phase 8C triggers live-only spray from catch crossings, using RowErg 2.2m and SkiErg 0.4m offsets; native spray replaces web `Math.random()` variation with deterministic catch-ordinal/seed mixing. Phase 8D uses zero/four/four/six droplets per side across low/medium/high/ultra, while BikeErg disables wake and spray at every tier. |
...
| `src/routes/replay/[id]` (UI, quality, and past-session selection) | `Sources/RowPlayStudio/Views/ReplayView.swift` + `Sources/RowPlayStudio/Views/Replay3D/RealityReplaySceneView.swift` | Phase 3 adds the SwiftUI Canvas replay surface with playback controls. Phase 8D adds the accessible 3D-only quality menu and adaptive-reduction status. Phase 10A adds explicit local past-session rival selection/removal, shared-elapsed-time 2D/3D ghost rendering, and live race-gap presentation. Phase 10B generalizes to constant-pace and imported rivals, finish verdict, and local race export/share. |
...
| `src/lib/server/export.ts` | `Sources/RowPlayCore/Export/WorkoutExport.swift` | Phase 5 ports CSV and JSON export formatting. TCX export added as native-only capability. |
...
| `src/routes/api/export` and `src/routes/api/export/[id]` | `Sources/RowPlayStudio/Views/WorkoutFileActionsView.swift` | Phase 5 wires current-workout CSV/JSON export through the native save panel. TCX export added as native-only capability. |
...
| (new — no web equivalent) | `Sources/RowPlayPlatform/AppPreferences.swift` | Settings wiring centralizes `demoModeEnabled`, `reduceReplayMotion`, and `preferredDistanceUnit`; Phase 8D persists only the selected `replayRenderQuality` ceiling with a medium default/corrupt fallback. Effective quality, governor state, metrics, and logs remain scene-local and ephemeral. |

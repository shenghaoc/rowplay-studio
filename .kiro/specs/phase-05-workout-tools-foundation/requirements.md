...
- **R1.2** `WorkoutComparison.compareVerdict(_:_:)` decides which workout was "better" for like-for-like distances (same band) by time, otherwise by average pace.
- **R1.3** `WorkoutSideStats` struct records time, pace, avgWatts, best5sPower, avgHr, peakHr, avgDps, and paceConsistency.
- **R1.4** `WorkoutComparison.sideStats(_:)` computes per-workout statistics from strokes and splits.
...
- **R2.2** `RepDetection.detectReps(_:)` returns one `RepSeries` per work interval, or nil when the workout is not a recognisable multi-rep piece (< 2 work intervals or each < 30 s).
...
- **R3.1** `WorkoutExport.csv(_:)` produces RFC 4180-compliant CSV with the same column order as the web export.
- **R3.2** `WorkoutExport.json(_:)` produces a JSON export with schema metadata (`rowplay-logbook-export`, version 1).
...
- **R4.6** `HrImport.applyHrImport(_:samples:offsetSec:)` produces a new `WorkoutDetail` with merged HR across strokes and splits.
...
- **R5.2** `AnnotationStore` protocol defines `loadAnnotations(workoutId:)`, `saveAnnotation(workoutId:_:)`, and `deleteAnnotation(workoutId:id:)` operations.

...
4. **Endpoint construction**: Deterministic URL building matching the web app's Concept2 API routes.
5. **Typed errors**: `Concept2Error` enum with transport, HTTP, decoding, and authentication failure cases. Error descriptions must not include tokens, headers, or raw payloads.
6. **Response decoding**: Minimal Codable models for the Concept2 logbook API response envelope and workout summary fields.
7. **Domain mapping**: Map raw API responses to existing `Workout`, `Stroke`, and `Split` domain types.
